...
class S2cPlayerPro:
    ...
    def download_bit(self, f1, f2=None):
        ...
        ret = Util.call(s2cdownload, ['-b', boardtype, '-f', download_xml , '-i', self.fpga.get_ip(), '--port', '8201'], timeout=600)
        '''

        def gen_download_xml(xml):
            with open(xml, 'w') as f:
                f.write(f'<?xml version="1.0" encoding="UTF-8" standalone="no"?>')
                f.write(f'<Download project="Runtime">\n')
                f.write(f'    <Module idx="0">\n')
        ...
        # generate download script for fpga board1
        if f1 != 'null':
            f1_flag = 'on'
            f2_flag = 'off'
            download_xml_1 = os.path.join(self.s2chome, 'board1_download_test_1.xml')
            gen_download_xml(download_xml_1)

        # generate download script for fpga board2
        if f2 != 'null':
            f1_flag = 'off'
            f2_flag = 'on'
            download_xml_2 = os.path.join(self.s2chome, 'board1_download_test_2.xml')
            gen_download_xml(download_xml_2)
        ...
        fpga_cnt = self.fpga.get_fpga_cnt()
        if fpga_cnt == 1:
            ret = Util.call(s2cdownload, ['-b', boardtype, '-f', download_xml_1 , '-i', self.fpga.get_ip()], timeout=600)
        else :
            if f1 != 'null':
                ret = Util.call(s2cdownload, ['-b', boardtype, '-f', download_xml_1 , '-i', self.fpga.get_ip(), '--port', '8200'], timeout=600)
            if f2 != 'null':
                ret = Util.call(s2cdownload, ['-b', boardtype, '-f', download_xml_2 , '-i', self.fpga.get_ip(), '--port', '8201'], timeout=600)
            if ret != 0:
                return 1
            return 0
