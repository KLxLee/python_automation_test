...
parser.add_option("-f", "--file", dest="yaml_filename", help="customized test yaml filename",
                  default="test_config.yml")
parser.add_option("-m", "--mode", dest="mode", help="reset, dl_only, off, default",
                  default="default")
...
if __name__ == '__main__':
    ...
    if mode == "dl_only":
        MODE_POWER_ON = False
        MODE_POWER_OFF = False
        MODE_SET_CLOCK = False
        MODE_DL_BITFILE = True
        MODE_LOAD_BATFILE = False
    elif mode == "reset":
        MODE_POWER_ON = True
        MODE_POWER_OFF = True
        MODE_SET_CLOCK = False
        MODE_DL_BITFILE = False
        MODE_LOAD_BATFILE = False
    elif mode == "off":
        MODE_POWER_ON = False
        MODE_POWER_OFF = True
    ...
    os.system("type " + working_path + "\\" + yaml_filename)

    with open(working_path + "\\" + yaml_filename) as file:
    ...
    for testName in data.keys():
        ...
        if MODE_DL_BITFILE:
            printStage("Player Pro - Download Bitfile")
            if not ppro.download_bit(f1=fpga.bitfile_fpga1, f2=fpga.bitfile_fpga2) == Util.SUCCESS:
                printStage("Player Pro - Download Bitfile - Error")
        
        if MODE_LOAD_BATFILE:
            printStage ("Running bat file")
